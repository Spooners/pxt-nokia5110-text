...
namespace display {
    ...
    MicroBitPin *Din   = getPin(MICROBIT_ID_IO_P15);
    MicroBitPin *Clk   = getPin(MICROBIT_ID_IO_P13);
    MicroBitPin *Light = getPin(MICROBIT_ID_IO_P16);
    MicroBitPin *Miso  = getPin(MICROBIT_ID_IO_P14); // Unused pin, needed by the SPI API

    SPI *spi = new SPI(Din->name, Miso->name, Clk->name);
    ...
    void initialize()
    {
        ...
        serial.send("configing\n");
        write(Command, 0x21);
        serial.send("configed\n");
        write(Command, 0xBF);
        write(Command, 0x04);
        write(Command, 0x14);
        write(Command, 0x0C);
    ...
    */
    ...
    //% advanced=true
    void changeSPIPins(DigitalPinEnum DIN, DigitalPinEnum CLK, DigitalPinEnum MISO) {
        initialized = false;
        ...
        delete spi;
        serial.send("SPI deleted\n");
        spi = new SPI(Din->name, Miso->name, Clk->name);
    ...
    */
    ...
    //% advanced=true
    void changeBusPins(DigitalPinEnum RST, DigitalPinEnum CE, DigitalPinEnum DC) {
        initialized = false;
        serial.send("bus changed\n");
        Rst = getPin((int)RST);
        Ce = getPin((int)CE);
        Dc = getPin((int)DC);
    ...
    */
    ...
    //% advanced=true
    void changeLightPin(DigitalPinEnum LIGHT) {
